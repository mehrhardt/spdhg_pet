...
for dataset in datasets:
    ...
    def save_image(x, n, f): misc.save_image(x.asarray(), n, f, planes=planes,
                                             clim=clim)
    ...
    for alpha, eta in [(a, e) for a in alphas for e in etas]:
        print('<<< <<< alpha = {}, eta = {}'.format(alpha, eta))

        folder_param = '{}/alpha{:.2g}_eta{:.2g}'.format(folder_main, alpha,
                                                         eta)
        ...
        if not os.path.exists('{}/gray_image_pet.png'.format(fldr)):
            ...
            misc.save_image(tmp.asarray(), 'image_norm_sideinfo_grad',
                            fldr, planes=planes, cmaps={'gray'},
                            clim=[0, 100])

            tmp = norm(xi)
            misc.save_image(tmp.asarray(), 'image_norm_vfield', fldr,
                            planes=planes)
        ...
        if not os.path.exists(file_target):
            ...
            norm_A = misc.norm(A, '{}/norm_dtv.npy'.format(folder_param))
            sigma = rho / norm_A
            tau = rho / norm_A
        ...
        for alg in algs:
            ...
            if os.path.exists(file_result):
                ...
                if alg.startswith('PDHG1'):
                    norm_A = misc.norm(A, '{}/norm_dtv.npy'
                                          .format(folder_param))
                    sigma = rho / norm_A
                    tau = rho / norm_A
                    ...
                    tau = rho * min([pi / nAi for pi, nAi in zip(prob, norm_Ai)])
